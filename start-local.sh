...
# Same as start-app.sh but runs on LOCALHOST only (no network exposure).
# Usage: ./start-local.sh
#   → http://localhost:5121 (Development profile; use appsettings.Development.json with RAG BaseUrl http://127.0.0.1:8010)
set -e

    ...
    public class HeyGenStreamingService : IHeyGenStreamingService
    {
        ...
        public async Task<HeyGenStreamingSessionData> CreateStreamingSessionAsync()
        {
            try
            {
                ...
                // NOTE: We use task_type: "repeat" when sending tasks to force verbatim speech
                // This prevents HeyGen's built-in LLM from processing our RAG responses
                var request = new
                {
                    version = "v2",
                    ...
                    quality = "high",
                    video_encoding = "H264"
                    // No system_prompt or knowledge base needed - we use "repeat" mode for verbatim TTS
                };
        ...
        public async Task StartStreamingSessionAsync(string sessionId, string? streamingToken = null)
        {
            ...
            try
            {
                ...
                {
                    var errorContent = await response.Content.ReadAsStringAsync();
                    // Don't throw - session might already be started/closed (we call start before every task; keepalive may call after close)
                    bool knownState = response.StatusCode == System.Net.HttpStatusCode.BadRequest && (
                        errorContent.IndexOf("session state: connected", StringComparison.OrdinalIgnoreCase) >= 0 ||
                        errorContent.IndexOf("session state: closed", StringComparison.OrdinalIgnoreCase) >= 0 ||
                        errorContent.IndexOf("already", StringComparison.OrdinalIgnoreCase) >= 0);
                    if (knownState)
                        _logger.LogDebug("HeyGen streaming.start 400 (expected when session already started/closed): {Error}", errorContent);
        ...
        public async Task SendStreamingTaskAsync(string sessionId, string text, string? streamingToken = null)
        {
            ...
            try
            {
                _logger.LogInformation(
                    "Sending streaming task to session {SessionId} in REPEAT mode (verbatim speech): {TextPreview}...",
                    sessionId, text.Substring(0, Math.Min(50, text.Length)));

                // ✅ CRITICAL: We REQUIRE the same session token that was used to create the session
                // Do NOT generate a new token - it must be the one from streaming.create_token
                if (string.IsNullOrWhiteSpace(streamingToken))
                {
                    _logger.LogError(
                        "Streaming token is REQUIRED for streaming.task - token from session creation must be reused. " +
                        "Session ID: {SessionId}", sessionId);
                    throw new InvalidOperationException(
                        "Streaming token is required for streaming.task – pass the token from streaming.create_token / session.create");
                }

                var token = streamingToken;
                _logger.LogInformation("Using streaming token from session creation (token length: {Length})", token.Length);

                // Use "repeat" mode to force verbatim speech - avatar will speak exactly what we send
                // This prevents HeyGen's built-in LLM from processing and generating its own response
                var request = new
                {
                    session_id = sessionId,
                    text = text,
                    task_type = "repeat",  // CRITICAL: Use "repeat" instead of "talk" to speak RAG response verbatim
                    task_mode = "sync"     // Synchronous mode for immediate response
                };

                var json = JsonSerializer.Serialize(request);
                _logger.LogInformation("HeyGen streaming.task request payload: {RequestJson}", json);
                ...
                // ✅ CRITICAL: Use Authorization: Bearer {token} for streaming.task
                // The token from streaming.create_token must be used, NOT X-Api-Key
                requestMessage.Headers.Authorization =
                    new System.Net.Http.Headers.AuthenticationHeaderValue("Bearer", token);

                _logger.LogInformation("Sending POST request to: {Url} with Bearer token", $"{_apiUrl}streaming.task");
                ...
                _logger.LogInformation(
                    "HeyGen streaming.task response - Status: {StatusCode}, Body: {Response}",
                    response.StatusCode, responseContent);

                // Parse response to check for task_id or other success indicators
                try
                {
                    using var responseDoc = JsonDocument.Parse(responseContent);
                    if (responseDoc.RootElement.TryGetProperty("data", out var dataElement) &&
                        dataElement.TryGetProperty("task_id", out var taskIdElement))
                    {
                        _logger.LogInformation("HeyGen task created successfully - Task ID: {TaskId}",
                            taskIdElement.GetString());
                    }
                }
                catch (Exception parseEx)
                {
                    _logger.LogWarning(parseEx, "Could not parse HeyGen response as JSON");
                }

                _logger.LogInformation(
                    "HeyGen streaming task sent successfully in REPEAT mode - avatar should speak RAG response verbatim");
            }
            catch (Exception ex)
            {
